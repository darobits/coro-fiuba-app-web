...
export default function JoinForm() {
  ...
  return <><section className="join-page-hero"><div><p className="section-index">05 — Contacto</p><p className="eyebrow"><span /> Tu voz también construye</p><h1>Hay un lugar<br />para <em>vos.</em></h1><p>Completá tus datos y nos pondremos en contacto para coordinar una audición de ingreso. La convocatoria está especialmente dirigida a estudiantes, docentes, graduados, graduadas y personal de la Facultad.</p></div><aside><div><small>Ensayos</small><strong>Todos los viernes<br />19:30 — 22:00 h</strong></div><div><small>Lugar</small><strong>Av. Paseo Colón 850</strong><a href="https://maps.app.goo.gl/p9ZfPAG2v7PEmbyP6" target="_blank" rel="noreferrer">Abrir en Maps ↗</a></div></aside></section>
    <section className="form-section"><div className="form-aside"><span>01</span><h2>Audición<br />de ingreso.</h2><p>La incorporación al Coro se realiza mediante una audición. Para participar se requiere:</p><ul className="audition-requirements"><li>Tener la voz sana.</li><li>Contar con una afinación adecuada.</li><li>Poder sostener una línea vocal dentro de una obra a varias voces.</li><li>Asumir el compromiso de asistir regularmente durante todo el año.</li></ul><p>Los campos marcados con <b>*</b> son obligatorios. Usaremos tus datos únicamente para contactarte por la convocatoria.</p></div><form ref={formRef} className="join-form modern" onSubmit={submit} noValidate aria-busy={sending}>
      ...
      <label className={errors.consent ? "consent invalid" : "consent"}><input name="consent" type="checkbox" onChange={() => clear("consent")} /><i aria-hidden="true">✓</i><span>Acepto que el Coro de la Facultad de Ingeniería UBA use estos datos para contactarme sobre la convocatoria.</span>{errors.consent && <em>{errors.consent}</em>}</label>
    ...
    {submissionState === "success" && <div className="dashboard-notice-backdrop application-success-backdrop" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) closeSuccess(); }}><section className="dashboard-notice application-success-modal" role="dialog" aria-modal="true" aria-labelledby="application-success-title" aria-describedby="application-success-copy"><span className="dashboard-notice-kicker">Mensaje recibido</span><div className="application-success-check" aria-hidden="true"><span /></div><h2 id="application-success-title">¡Gracias por contactarte<br /><em>con el Coro de la Facultad de Ingeniería UBA!</em></h2><p id="application-success-copy">Recibimos tu mensaje correctamente.</p><p className="application-success-note">Nos pondremos en contacto con vos a la brevedad.</p><button ref={closeButtonRef} className="button button-blue" type="button" onClick={closeSuccess}>Cerrar</button></section></div>}
  </>;
}
