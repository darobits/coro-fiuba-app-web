...
export default function JoinForm() {
  ...
  return <><section className="join-page-hero"><div><p className="section-index">05 — Contacto</p><p className="eyebrow"><span /> Tu voz también construye</p><h1>Hay un lugar<br />para <em>vos.</em></h1><p>Completá tus datos y nos pondremos en contacto. No hace falta pertenecer a la FIUBA ni tener experiencia coral previa.</p></div><aside><div><small>Ensayos</small><strong>Todos los viernes<br />19:30 — 22:00 h</strong></div><div><small>Lugar</small><strong>Av. Paseo Colón 850</strong><a href="https://maps.app.goo.gl/p9ZfPAG2v7PEmbyP6" target="_blank" rel="noreferrer">Abrir en Maps ↗</a></div></aside></section>
    <section className="form-section"><div className="form-aside"><span>01</span><h2>Contanos<br />sobre vos.</h2><p>Los campos marcados con <b>*</b> son obligatorios. Revisaremos tus datos únicamente para contactarte por la convocatoria.</p></div><form className="join-form modern" onSubmit={submit} noValidate>
      ...
      <div className="form-row"><label className={fieldClass("fullName")}><span>Nombre y apellido *</span><input name="fullName" autoComplete="name" placeholder="¿Cómo te llamás?" required onChange={() => clear("fullName")} aria-invalid={!!errors.fullName} />{errors.fullName && <em>{errors.fullName}</em>}</label><label className={fieldClass("email")}><span>Correo electrónico *</span><input name="email" type="email" inputMode="email" autoComplete="email" placeholder="tu@email.com" required maxLength={254} onChange={() => clear("email")} onBlur={event => checkEmail(event.currentTarget.value)} aria-invalid={!!errors.email} />{errors.email && <em>{errors.email}</em>}</label></div>
      ...
      <div className="form-submit"><button className="button button-blue button-submit" disabled={sending}>{sending ? "Enviando…" : "Enviar solicitud"}</button><p>Tu información se almacena de forma segura.</p></div>
